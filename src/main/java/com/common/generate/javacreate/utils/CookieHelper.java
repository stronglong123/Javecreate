...
public class CookieHelper {
	...
	public static final String YJP_COOKIE_ID = "YJPID";
     ...
     * @param domainUrl
     */
    public void deleteCookie(String name, String domainUrl) {
        Cookie c = getCookie(name);
        if (c != null) {
        	c.setPath("/");
        	c.setDomain(domainUrl);
            c.setMaxAge(0);
            addCookie(c);
        }
    }
    ...
    public Cookie getYjpCookie(boolean create) {
    	return getYjpCookie(create, null);
    }

    /**
     * 
     * 指定域名下易酒批专有 含通信标识 的 cookie
     * @param create
     * @param domain
     * @return
     */
    public Cookie getYjpCookie(boolean create, String domain) {
        Cookie cookie = getCookie(YJP_COOKIE_ID);
        if (null == cookie && create) {
            ...
            cookie = new Cookie(YJP_COOKIE_ID, yjpID);
            cookie.setPath("/");
            cookie.setHttpOnly(true);
            if (null != domain && !"".equals(domain)) {
            	cookie.setDomain(domain);
            }
            cookie.setMaxAge(7200); // 默认7200秒
            addCookie(cookie);
        }
